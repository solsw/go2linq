...
func Test_ElementAtErr_int(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ElementAtErr(tt.args.source, tt.args.idx)
			if (err != nil) != tt.wantErr {
				t.Errorf("ElementAtErr() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tt.wantErr {
				if err != tt.expectedErr {
					t.Errorf("ElementAtErr() error = '%v', expectedErr '%v'", err, tt.expectedErr)
				}
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ElementAtErr() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_ElementAtOrDefault_int(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ElementAtOrDefault(tt.args.source, tt.args.idx); !reflect.DeepEqual(got, tt.want) {
