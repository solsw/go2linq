...
func TestSequenceEqual_int(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "EmptyFirst",
			args: args{
				first:  Empty[int](),
				second: iterhelper.Var(2),
			},
			want: false,
		},
		{name: "EmptySecond",
			args: args{
				first:  iterhelper.Var(1),
				second: Empty[int](),
			},
			want: false,
		},
		...
		{name: "UnequalLengthsBothArrays",
			args: args{
				first:  iterhelper.Var(1, 5, 3),
				second: iterhelper.Var(1, 5, 3, 10),
			},
			want: false,
		},
	...
	tests := []struct {
		...
		{name: "4",
			args: args{
				...
				second: iterhelper.Var("a"),
			},
			want: false,
		},
		{name: "5",
			args: args{
				first:  iterhelper.Var("a"),
				second: iterhelper.Var("a", "b"),
			},
			want: false,
		},
	}
...
func TestSequenceEqualEq_string(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "CustomEqualityComparer",
			args: args{
				first:  iterhelper.Var("foo", "BAR", "baz"),
				second: iterhelper.Var("FOO", "bar", "Baz"),
				equal:  caseInsensitiveEqual,
			},
			want: true,
		},
